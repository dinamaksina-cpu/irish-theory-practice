...
function defaultProfile(name='Мій профіль'){return {name,favorites:[],mistakes:[],answered:[],correct:[],last:0,examScores:[],dailyActivity:{}}}
...
function choose(i){const q=session[index],s=sets();if(mode==='exam'){selected=i;examAnswers[index]=i;recordActivity(i===q.correctIndex);saveSets(s);render();return}selected=null;checkedAnswer=i;s.answered.add(q.id);const ok=i===q.correctIndex;recordActivity(ok);if(ok){s.mistakes.delete(q.id);s.correct.add(q.id)}else{s.mistakes.add(q.id);s.correct.delete(q.id)}saveSets(s);render()}
...
function finish(){if(mode!=='exam'){stopActivity();showHome();return}stopExamTimer();stopActivity();const elapsed=Math.min(45*60,Math.floor((Date.now()-examStartedAt)/1000));const score=session.reduce((sum,q,i)=>sum+(examAnswers[i]===q.correctIndex?1:0),0),s=sets();const wrong=[];session.forEach((q,i)=>{s.answered.add(q.id);if(examAnswers[i]===q.correctIndex){s.correct.add(q.id);s.mistakes.delete(q.id)}else{s.correct.delete(q.id);s.mistakes.add(q.id);wrong.push({position:i+1,q,chosen:examAnswers[i]})}});profile().examScores=profile().examScores||[];profile().examScores.push(score);saveSets(s);showExamResult(score,elapsed,wrong)}
