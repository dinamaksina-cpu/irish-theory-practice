...
function openInstallGuide(){const modal=$('#installGuideModal');if(!modal)return;const ua=navigator.userAgent||'';const isIOS=/iPhone|iPad|iPod/i.test(ua);const isAndroid=/Android/i.test(ua);$('#iosInstallGuide')?.classList.toggle('hidden',isAndroid&&!isIOS);$('#androidInstallGuide')?.classList.toggle('hidden',isIOS&&!isAndroid);$('#nativeInstallBtn')?.classList.toggle('hidden',!deferredInstallPrompt);modal.classList.remove('hidden');document.body.classList.add('modal-open')}
function closeInstallGuide(){$('#installGuideModal')?.classList.add('hidden');document.body.classList.remove('modal-open')}
