...
function openExamChoice(){const state=profile().incompleteExam;if(!state?.session?.length){start('exam',null,'new');return}const count=examAnsweredCount(state);$('#examResumeTitle').textContent=t('unfinishedExam');$('#examResumeText').textContent=t('completedOf').replace('{count}',count);$('#resumeExamBtn').textContent=t('resumeExam');$('#newExamBtn').textContent=t('startNewExam');$('#cancelExamChoice').textContent=t('cancel');$('#examResumeModal').classList.remove('hidden')}
