...
function updatePlanUI(){const badge=$('#planBadge');if(badge){badge.textContent=isPremium?t('premiumPlan'):t('freePlan');badge.classList.toggle('premium',isPremium)}const left=Math.max(0,FREE_EXAM_LIMIT-demoExamAttempts);const examHint=$('#examAccessHint');if(examHint)examHint.textContent=isPremium?(baseLang()==='en'?'Unlimited exams':baseLang()==='ru'?'Неограниченные экзамены':'Необмежені іспити'):`${t('demoExamsLeft')}: ${left}`;$$('[data-premium-only]').forEach(el=>el.classList.toggle('premium-locked',!isPremium));$$('[data-premium-section]').forEach(el=>el.classList.toggle('premium-section-locked',!isPremium));const allLabel=$('#allAccessHint');if(allLabel)allLabel.textContent=isPremium?`${questions.length} ${t('questions')}`:t('freeQuestions');}
