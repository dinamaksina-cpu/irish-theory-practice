...
function start(type,specificId,startAt){stopExamTimer();mode=type;selected=null;checkedAnswer=null;examAnswers=[];const s=sets();let source;if(type==='random20')source=shuffle(questions).slice(0,20);else if(type==='exam')source=shuffle(questions).slice(0,40);else if(type==='favorites')source=questions.filter(q=>s.favorites.has(q.id));else if(type==='mistakes')source=questions.filter(q=>s.mistakes.has(q.id));else source=questions;session=prepareSession(source);if(specificId){const pos=session.findIndex(q=>q.id===specificId);index=pos>=0?pos:0}else if(type==='all'&&startAt==='last'){index=Math.min(profile().last||0,Math.max(session.length-1,0))}else index=0;if(!session.length){alert(baseLang()==='ru'?'Список пуст.':baseLang()==='en'?'This list is empty.':'Список порожній.');return}if(type==='exam')startExamTimer();$('#homeView').classList.add('hidden');$('#quizView').classList.remove('hidden');$('#quizView').classList.toggle('exam-mode',type==='exam');render();window.scrollTo({top:0,behavior:'instant'})}
