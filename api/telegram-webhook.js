...
module.exports = async function handler(req, res) {
  ...
  if (configuredSecret) {
    ...
    if (receivedSecret !== configuredSecret) return res.status(401).json({ ok: false, error: 'Invalid webhook secret' });
  }
  ...
  try {
    ...
    if (!supported.includes(command)) return res.status(200).json({ ok: true, ignored: true });
